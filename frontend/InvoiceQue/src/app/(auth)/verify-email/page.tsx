...
import { CancelCircleIcon, CheckmarkCircle01Icon, Loading01Icon } from 'hugeicons-react';
  ...
  useEffect(() => {
    ...
    async function verify() {
      ...
      try {
        ...
        setState('success');
        setMessage(res.message || 'Email berhasil diverifikasi. Silakan login.');
      } catch (err) {
        setState('error');
    ...
    <div className="min-h-screen bg-bg-secondary flex items-center justify-center p-6">
      <div className="w-full max-w-[480px] bg-bg-card border border-border-color rounded-xl p-8 shadow-xl text-center">
        ...
        <h1 className="text-2xl font-extrabold mb-2">
          {state === 'success' ? 'Email Terverifikasi' : state === 'error' ? 'Verifikasi Gagal' : 'Mohon Tunggu'}
        </h1>
        <p className="text-sm text-text-secondary leading-relaxed mb-6">{message}</p>

        {state === 'success' ? (
          <Link href="/login?verified=1" className="btn btn-primary w-full">Login Sekarang</Link>
        ) : state === 'error' ? (
          <div className="flex flex-col gap-2">
            <Link href="/login" className="btn btn-primary w-full">Ke Login</Link>
            <Link href="/register" className="btn btn-secondary w-full">Daftar Ulang</Link>
          </div>
        ) : (
          <div className="text-xs text-text-tertiary">Jangan tutup halaman ini.</div>
        )}
      </div>
    </div>
  );
