...
const getCheckoutError = (json: CheckoutErrorResponse, fallback: string) => {
  return (
    json?.details?.message || json?.details?.error || json?.error || fallback
  );
};
...
export default function PublicPaymentPage() {
  ...
  if (isCompletedStatus(data?.status)) {
    return (
      <div
        className="min-h-screen bg-bg-secondary flex items-center justify-center p-6"
        dir={t.dir}
      >
        <div className="w-full max-w-[520px]">
          ...
          <div className="bg-bg-card border border-border-color rounded-xl p-9 max-sm:p-6 shadow-xl text-center">
            ...
            <p className="text-text-secondary">
              {t.thankYouPaymentReceived} <strong>{title}</strong>{" "}
              {locale === "ar"
                ? "."
                : locale === "zh" || locale === "ja"
                  ? "の支払い。"
            ...
            <div className="text-3xl font-black text-emerald-600 mt-4">
  ...
  return (
    <div
      className="min-h-screen bg-bg-secondary flex items-center justify-center p-4 sm:p-8 md:p-12 font-sans"
      dir={t.dir}
    >
        ...
        <div className="bg-white dark:bg-[#121212] border border-border-color rounded-sm shadow-2xl relative">
          ...
          <div className="p-6 sm:p-10 md:p-14 pt-10 sm:pt-14 md:pt-16">
            ...
            >
              ...
              <div
                className={`${isRTL ? "text-right sm:text-left" : "text-left sm:text-right"}`}
              >
                <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-text-primary tracking-tighter uppercase mb-1">
                  {t.invoice}
                </h1>
            ...
            <div className="flex flex-col sm:flex-row justify-between border-y border-border-light py-6 mb-8 gap-6">
              <div className="flex-1">
                ...
                {provider && provider !== "manual" && (
                  <span
                    className={`inline-block mt-1 py-0.5 px-2 rounded text-[11px] font-bold uppercase tracking-wider ${
                      provider === "paypal"
              ...
              <div className={`${isRTL ? "" : "sm:text-right"}`}>
                ...
                <p className="text-3xl md:text-4xl font-black text-red-600 dark:text-red-500">
                  {formatAmount(amount, currency)}
                </p>
              </div>
            </div>
            ...
            <div className="mb-10">
              <div
                className={`hidden sm:grid grid-cols-12 gap-4 pb-3 border-b-2 border-border-color text-xs font-bold text-text-tertiary uppercase tracking-wider`}
              >
                <div className="col-span-8">{t.itemDescription}</div>
                <div
                  className={`col-span-4 ${isRTL ? "text-left" : "text-right"}`}
                >
                  {t.amount}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 py-5 border-b border-border-light">
                <div className="col-span-1 sm:col-span-8">
                  <p className="font-bold text-text-primary text-base sm:text-lg mb-1">
                    {title}
                  </p>
                  {description && (
                ...
                <div
                  className={`col-span-1 sm:col-span-4 ${isRTL ? "sm:text-left" : "sm:text-right"} font-bold text-text-primary self-center text-lg`}
                >
                  {formatAmount(amount, currency)}
                </div>
              </div>
            ...
            {/* Summary Section */}
            <div
              className={`flex ${isRTL ? "justify-start" : "justify-end"} mb-12`}
            >
            ...
            <div className="bg-bg-secondary/50 rounded-xl p-6 sm:p-8 border-2 border-dashed border-border-color">
              <h3 className="text-lg font-extrabold text-center mb-6">
                ...
                <div className="max-w-xl mx-auto">
                  ...
                  <div className="mb-6">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      <label
                        className={`flex flex-col items-center justify-center gap-2 py-4 px-3 border-2 rounded-xl cursor-pointer transition-all duration-200 hover:border-red-300 ${paymentMethod === "transfer" ? "border-red-500 bg-red-50 dark:bg-red-900/10 shadow-sm" : "border-border-color bg-white dark:bg-bg-card"}`}
                      >
                        <input
                          type="radio"
                          name="method"
                      ...
                      <label
                        className={`flex flex-col items-center justify-center gap-2 py-4 px-3 border-2 rounded-xl cursor-pointer transition-all duration-200 hover:border-red-300 ${paymentMethod === "ewallet" ? "border-red-500 bg-red-50 dark:bg-red-900/10 shadow-sm" : "border-border-color bg-white dark:bg-bg-card"}`}
                      >
                        <input
                          type="radio"
                          name="method"
                      ...
                      <label
                        className={`flex flex-col items-center justify-center gap-2 py-4 px-3 border-2 rounded-xl cursor-pointer transition-all duration-200 hover:border-red-300 ${paymentMethod === "va" ? "border-red-500 bg-red-50 dark:bg-red-900/10 shadow-sm" : "border-border-color bg-white dark:bg-bg-card"}`}
                      >
                        <input
                          type="radio"
                          name="method"
                      ...
                      <label
                        className={`flex flex-col items-center justify-center gap-2 py-4 px-3 border-2 rounded-xl cursor-pointer transition-all duration-200 hover:border-red-300 ${paymentMethod === "qris" ? "border-red-500 bg-red-50 dark:bg-red-900/10 shadow-sm" : "border-border-color bg-white dark:bg-bg-card"}`}
                      >
                        <input
                          type="radio"
                          name="method"
                  ...
                  {paymentMethod === "transfer" && (
                    <div className="mb-6">
                      <div className="p-5 bg-white dark:bg-bg-card border border-border-color rounded-xl flex flex-col gap-3 shadow-sm">
                        <h4 className="text-sm font-bold border-b border-border-light pb-2 mb-1">
