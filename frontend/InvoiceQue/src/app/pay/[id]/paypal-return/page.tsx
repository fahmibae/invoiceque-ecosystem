...
export default function PayPalReturnPage() {
  ...
  useEffect(() => {
    ...
    const captureAndVerify = async () => {
      try {
        // Step 1: Fetch payment info for display
        const infoRes = await fetch(`${API_BASE}/pay/${params.id}`);
        if (infoRes.ok) {
        ...
        if (captureRes.ok && captureData.status === "completed") {
          ...
          // Refresh payment info after successful capture
          const refreshRes = await fetch(`${API_BASE}/pay/${params.id}`);
          if (refreshRes.ok) {
            const refreshData = await refreshRes.json();
            setPaymentInfo({
              title: refreshData.title || "Payment",
              amount: refreshData.amount || 0,
              currency: refreshData.currency || "USD",
              status: refreshData.status,
            });
          }
        } else if (captureRes.ok) {
          // Capture returned non-completed status — fallback to polling
    ...
    // Fallback polling if capture returns a non-completed status
      ...
      const poll = async (): Promise<void> => {
        try {
          ...
          if (attempts < maxAttempts) {
            ...
            // After max attempts, show success anyway — webhook will finalize
            setStatus("success");
          }
        } catch {
          attempts++;
      ...
      <div className="w-full max-w-[520px]">
        ...
        <div className="bg-bg-card border border-border-color rounded-xl p-9 max-sm:p-6 shadow-xl text-center relative overflow-hidden">
          ...
          <div className="flex items-center justify-center gap-2.5 mb-7">
            <div className="w-10 h-10 bg-gradient-to-br from-red-600 to-red-500 rounded-sm flex items-center justify-center font-extrabold text-[15px] text-white">
              IQ
            </div>
            <span className="text-lg font-extrabold bg-gradient-to-br from-red-600 to-red-500 bg-clip-text text-transparent">
              InvoiceQu
            </span>
          </div>
          ...
          {status === "success" && (
            <>
              ...
              <h1 className="text-2xl font-extrabold mb-2 text-emerald-600">
                {t.paymentSuccessful}
              </h1>
              {paymentInfo && (
                <>
          ...
          {status === "error" && (
            <>
              ...
              <p className="text-sm text-text-secondary mt-2">
                {errorMessage || t.cannotVerifyPayment}
              </p>
              <button
          ...
          <div className="text-center text-xs text-text-tertiary pt-4 border-t border-border-light">
            <span>
              Powered by <strong>InvoiceQu</strong>
            </span>
          </div>
        </div>
      </div>
