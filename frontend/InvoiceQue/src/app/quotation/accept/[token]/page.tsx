...
type PublicQuotation = Quotation & { accent_color?: string; business_name?: string; business_logo?: string };
...
export default function AcceptQuotationPage() {
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      setQuotation(prev => ({ ...updated, accent_color: prev?.accent_color }));
    ...
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex items-center justify-center p-6">
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-10 text-center max-w-md w-full border border-slate-200 dark:border-slate-700">
          <div className="text-5xl mb-4"><CancelCircleIcon size={48} color='#ff0000' /></div>
          <h2 className="text-xl font-bold mb-2 text-slate-900 dark:text-white">Quotation Not Found</h2>
          <p className="text-slate-500 text-sm">{error || 'This quotation link is invalid or has expired.'}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex items-center justify-center p-6">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl max-w-lg w-full overflow-hidden border border-slate-200 dark:border-slate-700">
        ...
        <div className="bg-gradient-to-br from-red-600 to-red-500 p-6 text-white text-center">
          ...
          <p className="text-red-100 text-sm mt-1">{quotation.quotation_number}</p>
        </div>
        ...
        <div className="p-6">
          ...
          <div className="mb-5 p-4 bg-slate-50 dark:bg-slate-700/50 rounded-lg">
            <div className="text-[11px] font-semibold text-slate-500 uppercase tracking-wide mb-1">Prepared for</div>
            <div className="font-bold text-slate-900 dark:text-white">{quotation.client_name}</div>
            <div className="text-sm text-slate-500">{quotation.client_email}</div>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-200 dark:border-slate-600">
                  ...
                  <th className="py-2 text-center text-xs font-semibold text-slate-500 uppercase">Qty</th>
              ...
              <tbody>
                {quotation.items.map((item, idx) => (
                  <tr key={idx} className="border-b border-slate-100 dark:border-slate-700">
                    <td className="py-2.5 text-slate-900 dark:text-white">{item.description}</td>
                    <td className="py-2.5 text-center text-slate-500">{item.quantity}</td>
                    <td className="py-2.5 text-right font-semibold text-slate-900 dark:text-white">{formatCurrency(item.total, quotation.currency)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="border-t border-slate-200 dark:border-slate-600 pt-4 mb-6">
            <div className="flex justify-between text-sm text-slate-500 py-1"><span>Subtotal</span><span>{formatCurrency(quotation.subtotal, quotation.currency)}</span></div>
            {quotation.tax > 0 && <div className="flex justify-between text-sm text-slate-500 py-1"><span>Tax</span><span>{formatCurrency(quotation.tax, quotation.currency)}</span></div>}
            {quotation.discount > 0 && <div className="flex justify-between text-sm text-slate-500 py-1"><span>Discount</span><span className="text-emerald-600">-{formatCurrency(quotation.discount, quotation.currency)}</span></div>}
            <div className="flex justify-between text-xl font-extrabold pt-3 mt-2 border-t-2 border-red-500" style={{ color: accentColor }}>
              <span>Total</span>
          ...
          {quotation.valid_until && (
            <p className="text-xs text-slate-500 text-center mb-4">
              Valid until: <strong>{formatPublicDate(quotation.valid_until)}</strong>
            </p>
          )}
          ...
          {actionDone === 'accepted' ? (
            <div className="text-center p-6 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl border border-emerald-200 dark:border-emerald-800">
              <div className="text-5xl mb-2 flex justify-center text-emerald-600 dark:text-emerald-400"><CheckmarkBadge03Icon size={48} /></div>
              <h3 className="text-lg font-bold text-emerald-700 dark:text-emerald-400">Quotation Accepted</h3>
              <p className="text-sm text-emerald-600 dark:text-emerald-500 mt-1">Thank you. You have accepted this quotation.</p>
            </div>
          ) : actionDone === 'rejected' ? (
            <div className="text-center p-6 bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-200 dark:border-red-800">
              <div className="text-4xl mb-2"><CancelCircleIcon size={48} color='white' /></div>
              <h3 className="text-lg font-bold text-red-700 dark:text-red-400">Quotation Rejected</h3>
              <p className="text-sm text-red-600 dark:text-red-500 mt-1">You have rejected this quotation.</p>
            </div>
            ...
            <div className="flex gap-3">
              ...
              >
                {actionLoading ? '...' : <><div className="flex items-center justify-center gap-2"><CheckmarkBadge03Icon size={16} color='white' /> Accept</div></>}
              </button>
              <button
              ...
              >
                {actionLoading ? '...' : <><div className="flex items-center justify-center gap-2"><CancelCircleIcon size={16} color='white' /> Reject</div></>}
              </button>
            </div>
          ) : (
            <div className="text-center p-4 bg-slate-100 dark:bg-slate-700 rounded-xl">
              <p className="text-sm text-slate-500">This quotation is <strong>{quotation.status}</strong> and can no longer be changed.</p>
            </div>
          )}
        </div>
