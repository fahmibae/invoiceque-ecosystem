          ...
          <div className="card">
            ...
            <div className="form-group" style={{ marginTop: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button
                  type="button"
                  className={`btn ${paymentType === 'dp' ? 'btn-primary' : 'btn-secondary'}`}
                  style={{ flex: 1 }}
            ...
            {paymentType === 'dp' && (
              <div style={{ marginTop: 12, padding: 16, background: 'rgba(217,119,6,0.08)', borderRadius: 10, border: '1px solid rgba(217,119,6,0.2)' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 10, fontSize: 13 }}>
                  <div>
                    <span style={{ color: 'var(--text-tertiary)' }}>DP: </span>
                    <strong style={{ color: '#d97706' }}>{formatCurrency(dpAmount)}</strong>
                  </div>
                  <div>
                    <span style={{ color: 'var(--text-tertiary)' }}>Sisa: </span>
          ...
          <div className={`card ${styles.previewCard}`}>
            <div className={styles.previewHeader}>
              <div>
                ...
                <h2 className={styles.previewTitle}>INVOICE</h2>
              </div>
              <div className={styles.previewMeta}>
                <div className={styles.previewNum}>INV-XXXX-XXX</div>
                <div className={styles.previewDate}>Tanggal: {new Date().toLocaleDateString('id-ID')}</div>
                {dueDate && <div className={styles.previewDate}>Jatuh Tempo: {new Date(dueDate).toLocaleDateString('id-ID')}</div>}
              </div>
            </div>

            {client && (
              <div className={styles.previewClient}>
                <div className={styles.previewLabel}>Tagihan untuk:</div>
                <div style={{ fontWeight: 600 }}>{client.name}</div>
                <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{client.company}</div>
            ...
            <div className={styles.previewTotals}>
              ...
              {paymentType === 'dp' && (
                <>
                  <div className={styles.totalRow} style={{ marginTop: 8, paddingTop: 8, borderTop: '1px dashed rgba(217,119,6,0.3)' }}>
                    <span style={{ color: '#d97706' }}>DP ({dpPercentage}%)</span>
                    <span style={{ color: '#d97706', fontWeight: 700 }}>{formatCurrency(dpAmount)}</span>
                  </div>
                  <div className={styles.totalRow}>
            ...
            {notes && (
              <div className={styles.previewNotes}>
                <div className={styles.previewLabel}>Catatan:</div>
                <p>{notes}</p>
              </div>
            )}
