...
export default function OutstandingInvoicesPage() {
  ...
  async function fetchOutstanding() {
    try {
      ...
      setError(
        err instanceof Error ? err.message : t("invoices.outstanding.loadError"),
      );
    } finally {
      setLoading(false);
    }
  ...
  function openCreateLinkModal(inv: Invoice) {
    setModalInvoice(inv);
    setLinkTitle(
      t("invoices.outstanding.linkTitleTemplate", {
        number: inv.number,
        client: inv.client_name,
      }),
    );
    setLinkDescription(
      t("invoices.outstanding.linkDescriptionTemplate", {
        client: inv.client_name,
      }),
    );
  ...
  async function handleCreatePaymentLink() {
    ...
    try {
      ...
      setShowModal(false);
      setSuccessMsg(
        t("invoices.outstanding.successMessage", {
          number: modalInvoice.number,
        }),
      );
      ...
      alert(
        err instanceof Error
          ? err.message
          : t("invoices.outstanding.createLinkError"),
      );
    } finally {
  ...
  if (loading) {
    return (
      <div className="animate-fade-in p-10 text-center text-text-secondary flex flex-col items-center justify-center min-h-[400px]">
        ...
        <p>{t("invoices.outstanding.loading")}</p>
      </div>
    );
  }
    ...
    <div className="animate-fade-in">
      <Portal>
        ...
        {showModal && modalInvoice && (
          <div
            className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[1000] flex items-end sm:items-center justify-center p-0 sm:p-5"
            onClick={() => setShowModal(false)}
          >
            <div
              className="bg-bg-card w-full sm:max-w-[520px] sm:rounded-2xl rounded-t-2xl shadow-xl overflow-hidden border border-border-color animate-fade-in max-h-[90vh] flex flex-col"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Modal Header */}
              <div className="flex items-center justify-between px-6 py-4 border-b border-border-light shrink-0">
                <h3 className="text-lg font-bold flex items-center gap-2">
                  <LinkSquare01Icon width={20} height={20} />{" "}
                  {t("invoices.outstanding.modalTitle")}
                </h3>
                <button
              ...
              <div className="overflow-y-auto flex-1 px-6 py-5 flex flex-col gap-5">
                ...
                <div className="p-4 rounded-xl bg-gradient-to-br from-amber-50/80 to-amber-100/40 dark:from-amber-900/20 dark:to-amber-800/10 border border-amber-200/50 dark:border-amber-800/30">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-[13px]">
                    ...
                    <div>
                      <span className="text-text-tertiary">
                        {t("invoices.paid")} (DP {modalInvoice.dp_percentage}%)
                      </span>
                      <div className="font-semibold text-emerald-600">
                        {formatCurrency(
                          modalInvoice.amount_paid,
                          modalInvoice.currency,
                        )}
                      </div>
                    </div>
                  ...
                  <div className="mt-3">
                    <div className="flex justify-between text-[11px] text-text-tertiary mb-1">
                      <span>{t("invoices.outstanding.paymentProgress")}</span>
                      <span className="font-semibold">
                        {Math.round(
                          (modalInvoice.amount_paid / modalInvoice.total) * 100,
                        )}
                        %
                      </span>
                    </div>
                    <div className="h-2 bg-white/60 dark:bg-white/10 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${(modalInvoice.amount_paid / modalInvoice.total) * 100}%`,
                        }}
                      />
                    </div>
                  </div>
                ...
                <div className="form-group">
                  <label className="form-label">
                    {t("invoices.outstanding.paymentLinkTitle")}
                  </label>
                  <input
                    type="text"
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="form-group">
                    <label className="form-label">
                      {t("invoices.outstanding.paymentProvider")}
                    </label>
                    <select
                      className="form-input"
                      value={linkProvider}
                    ...
                    <p className="text-[10px] text-text-tertiary mt-1">
                      {linkProvider === "paypal"
                        ? t("invoices.outstanding.paypalHint")
                        : t("invoices.outstanding.xenditHint")}
                    </p>
                  </div>
                </div>
                <div className="form-group">
                  <label className="form-label">
                    {t("invoices.outstanding.billAmount", {
                      currency: linkCurrency,
                    })}
                  </label>
                  <input
                    ...
                    value={linkAmount}
                    onChange={(e) =>
                      setLinkAmount(parseFloat(e.target.value) || 0)
                    }
                  />
                  <p className="text-[11px] text-text-tertiary mt-1">
                    {t("invoices.outstanding.amountHint")}
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-3 px-6 py-4 border-t border-border-light shrink-0">
                ...
                <button
                  ...
                  disabled={
                    creatingLinkFor === modalInvoice.id || linkAmount <= 0
                  }
                >
                  {creatingLinkFor === modalInvoice.id ? (
                    ...
                    <>
                      <LinkSquare01Icon width={16} height={16} />{" "}
                      {t("invoices.outstanding.createPaymentLink")}
                    </>
                  )}
                </button>
              </div>
      ...
      <div className="page-header">
        <div className="page-header-left">
          <h1 className="page-title flex items-center gap-3">
            {t("invoices.outstanding.title")}
          </h1>
          <p className="page-subtitle">
            {t("invoices.outstanding.subtitle")}
          </p>
        </div>
        <Link
          href="/invoices"
      ...
      <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 max-sm:gap-2.5 mb-6">
        ...
        <div className="bg-bg-card border border-border-color rounded-lg p-5 flex items-start gap-4 relative overflow-hidden transition-all duration-200 shadow-sm hover:shadow-md hover:-translate-y-0.5 after:content-[''] after:absolute after:top-0 after:right-0 after:w-20 after:h-20 after:bg-gradient-to-br after:from-red-500/10 after:to-transparent after:rounded-bl-full">
          ...
          <div className="flex flex-col gap-1 flex-1 min-w-0 z-10">
            <span className="text-xs text-text-tertiary font-medium">
              {t("invoices.outstanding.totalOutstanding")}
            </span>
            <span className="text-xl font-extrabold text-text-primary tracking-tight max-lg:text-base whitespace-nowrap overflow-hidden text-ellipsis">
        ...
        <div className="bg-bg-card border border-border-color rounded-lg p-5 flex items-start gap-4 relative overflow-hidden transition-all duration-200 shadow-sm hover:shadow-md hover:-translate-y-0.5 after:content-[''] after:absolute after:top-0 after:right-0 after:w-20 after:h-20 after:bg-gradient-to-br after:from-green-500/10 after:to-transparent after:rounded-bl-full">
          ...
          <div className="flex flex-col gap-1 flex-1 min-w-0 z-10">
            <span className="text-xs text-text-tertiary font-medium">
              {t("invoices.outstanding.totalDpReceived")}
            </span>
            <span className="text-xl font-extrabold text-text-primary tracking-tight max-lg:text-base whitespace-nowrap overflow-hidden text-ellipsis">
        ...
        <div className="bg-bg-card border border-border-color rounded-lg p-5 flex items-start gap-4 relative overflow-hidden transition-all duration-200 shadow-sm hover:shadow-md hover:-translate-y-0.5 after:content-[''] after:absolute after:top-0 after:right-0 after:w-20 after:h-20 after:bg-gradient-to-br after:from-blue-500/10 after:to-transparent after:rounded-bl-full">
          ...
          <div className="flex flex-col gap-1 flex-1 min-w-0 z-10">
            <span className="text-xs text-text-tertiary font-medium">
              {t("invoices.outstanding.totalInvoiceValue")}
            </span>
            <span className="text-xl font-extrabold text-text-primary tracking-tight max-lg:text-base whitespace-nowrap overflow-hidden text-ellipsis">
      ...
      <div className="flex gap-2 mb-6">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder={t("invoices.outstanding.searchPlaceholder")}
      ...
      {filtered.length > 0 ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-3">
                      <Link
                        ...
                        className="text-base font-bold text-red-600 hover:text-red-700 transition-colors no-underline"
                      >
                        {inv.number}
                      </Link>
                      <span className="inline-block py-0.5 px-2.5 rounded-full text-[11px] font-semibold bg-amber-500/15 text-amber-600">
                      ...
                      <div className="flex-1">
                        <div className="flex justify-between items-center mb-1">
                          <span className="text-text-tertiary text-xs">
                            {t("invoices.outstanding.progress")}
                          </span>
                    ...
                    {inv.due_date && (
                      <div className="text-xs text-text-tertiary">
                        {t("invoices.outstanding.dueDate")}:{" "}
                  ...
                  <div className="lg:w-[320px] shrink-0 flex flex-col gap-3">
                    ...
                    <div className="flex gap-2">
                      {hasRemainingLink ? (
                        ...
                        >
                          <LinkSquare01Icon width={16} height={16} />{" "}
                          {t("invoices.outstanding.viewPaymentLink")}
                        </a>
                      ) : (
                        <button
                        ...
                        >
                          {creatingLinkFor === inv.id ? (
                            ...
                            <>
                              <Payment01Icon width={16} height={16} />{" "}
                              {t("invoices.outstanding.createRemainingLink")}
                            </>
                          )}
                        </button>
                      )}
        ...
        <div className="card flex flex-col justify-center items-center text-center py-16 px-5">
          ...
          <h3 className="text-lg font-semibold mb-2">
            {t("invoices.outstanding.emptyTitle")}
          </h3>
          <p className="text-sm text-text-secondary mb-6 max-w-md">
            {t("invoices.outstanding.emptySubtitle")}
          </p>
          <Link href="/invoices" className="btn btn-primary">
            {t("invoices.outstanding.viewAllInvoices")}
          </Link>
        </div>
      )}
    </div>
