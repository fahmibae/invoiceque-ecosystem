...
const statusConfig: Record<string, { labelKey: TranslationKey; cls: string }> =
  {
    active: { labelKey: "projects.status.active", cls: "badge-success" },
    completed: { labelKey: "projects.status.completed", cls: "badge-info" },
    on_hold: { labelKey: "projects.status.onHold", cls: "badge-warning" },
    cancelled: {
      labelKey: "projects.status.cancelled",
      cls: "badge-default",
    },
  };
...
export default function ProjectsPage() {
  ...
  const [completedProject, setCompletedProject] = useState<Project | null>(
    null,
  );
  ...
  const getProjectStats = (projectName: string) => {
    ...
    const totalAmount = projectTasks.reduce(
      (s, t) => s + (t.hourly_rate || 0) * (t.estimated_hours || 0),
      0,
    );
    return {
      total: totalCount,
      done,
      progress: totalCount > 0 ? Math.round((done / totalCount) * 100) : 0,
      totalHours,
      totalAmount,
    };
  };
  ...
  const addProject = async () => {
    ...
    try {
      await projectApi.create({
        ...
        client_name: newProject.client_name || t("projects.noClient"),
  ...
  const openProjectBilling = (kind: "quotation" | "invoice") => {
    ...
    router.push(
      kind === "quotation"
        ? `/quotations/create?${query}`
        : `/invoices/create?${query}`,
    );
  };
    ...
    <div className="animate-fade-in">
              ...
              <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                >
                  ...
                  <div className="font-bold text-text-primary">
                    {t("projects.createQuotation")}
                  </div>
                  <div className="text-xs text-text-secondary mt-1">
                    {t("projects.quotationHint")}
                  </div>
                </button>
                <button
                  type="button"
                ...
                >
                  ...
                  <div className="font-bold text-text-primary">
                    {t("projects.createInvoice")}
                  </div>
                  <div className="text-xs text-text-secondary mt-1">
                    {t("projects.invoiceHint")}
                  </div>
                </button>
              </div>
      ...
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={() => setShowModal(true)}>
          <Add01Icon width={16} height={16} /> {t("projects.new")}
        </button>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {projects.map((project) => {
            ...
            return (
              <div
                key={project.id}
                className="bg-bg-card border border-border-color rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1 group"
              >
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span
                      className={`badge ${sc.cls} text-[11px] shrink-0 ml-2`}
                    >
                      {t(sc.labelKey)}
                    </span>
                  </div>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="h-2 bg-bg-secondary rounded-full overflow-hidden">
                      <div
                        className="h-full rounded-full transition-all duration-500 bg-gradient-to-r from-red-500 to-red-400"
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    <div className="bg-bg-secondary rounded-lg p-2.5 text-center">
                      ...
                      <div className="text-sm font-bold">
                        {project.currency}{" "}
                        {project.budget.toLocaleString(intlLocale)}
                      </div>
                    </div>
                    <div className="bg-bg-secondary rounded-lg p-2.5 text-center">
                      <div className="text-xs text-text-tertiary mb-0.5">
                      ...
                      <div className="text-sm font-bold">
                        {project.currency}{" "}
                        {project.hourly_rate.toLocaleString(intlLocale)}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-border-light">
                    <div className="flex items-center gap-3 text-text-tertiary text-[11px] font-medium">
                      {daysLeft !== null && (
                        <span
                          className={`flex items-center gap-1 ${daysLeft < 0 ? "text-red-500" : daysLeft <= 7 ? "text-amber-500" : ""}`}
                        >
                          ...
                          {daysLeft < 0
                            ...
                            : t("projects.daysLeft", { count: daysLeft })}
                        </span>
                      )}
                      <span className="flex items-center gap-1">
                        <Clock01Icon width={12} height={12} />{" "}
                        {t("projects.hoursShort", {
                    ...
                    <div className="flex gap-1">
                      ...
                      <button
                        className="w-7 h-7 flex items-center justify-center rounded-md hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors text-text-tertiary hover:text-red-500"
                        onClick={() => {
            ...
            >
              <div className="flex items-center justify-between px-6 py-4 border-b border-border-light shrink-0">
                <h3 className="text-lg font-bold flex items-center gap-2">
                  <Folder01Icon width={20} height={20} /> {t("projects.new")}
                </h3>
                <button
              ...
              <div className="overflow-y-auto flex-1 px-6 py-5 flex flex-col gap-4">
                ...
                <div>
                  ...
                  <textarea
                    className="w-full py-2.5 px-3 border border-border-color rounded-lg bg-bg-secondary text-sm outline-none focus:border-red-400 resize-none"
                    rows={2}
                ...
                <div>
                  ...
                  >
                    <option value="">{t("projects.chooseClient")}</option>
                    {clients.map((c) => (
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewProject((p) => ({ ...p, currency: e.target.value }))
                    }
                  >
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-text-tertiary uppercase tracking-[0.5px] mb-2">
                      {t("projects.budget")} ({newProject.currency})
                    </label>
                    <input
                      type="number"
                      className="w-full py-2.5 px-3 border border-border-color rounded-lg bg-bg-secondary text-sm outline-none focus:border-red-400"
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-text-tertiary uppercase tracking-[0.5px] mb-2">
                      {t("projects.hourlyRate")} ({newProject.currency})
                    </label>
                    <input
                      type="number"
                      className="w-full py-2.5 px-3 border border-border-color rounded-lg bg-bg-secondary text-sm outline-none focus:border-red-400"
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {projectColors.map((c) => (
                      <button
                        key={c}
                        className={`w-8 h-8 rounded-full transition-all duration-150 ${newProject.color === c ? "ring-2 ring-offset-2 ring-red-500 scale-110" : "hover:scale-110"}`}
                        style={{ background: c }}
                        onClick={() =>
                          setNewProject((p) => ({ ...p, color: c }))
                        }
                      />
                    ))}
                  </div>
