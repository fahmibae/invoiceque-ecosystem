...
import { MoneyBag02Icon, GoogleDocIcon, Loading01Icon, Payment01Icon, ChartIcon, FlashIcon, Settings01Icon, UserGroup02Icon, LockedIcon } from 'hugeicons-react';
    ...
    <div className="animate-fade-in">
      <div className="page-header">
        <div className="page-header-left">
          <h1 className="page-title">Ringkasan Invoice Anda</h1>
          <p className="page-subtitle">Selamat datang kembali! Berikut ringkasan invoice Anda.</p>
        </div>
        {invoiceLocked ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] gap-5 mb-6">
        ...
        <div className="card flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-2.5 flex-1">
            {invoiceLocked ? (
              <button className="flex items-center gap-3.5 px-4 py-3.5 rounded-md bg-bg-secondary border border-border-light text-sm font-medium text-text-secondary opacity-70 cursor-not-allowed text-left" disabled title={subscription.limitMessage('invoices')}>
                <div className="w-9 h-9 rounded-sm flex items-center justify-center shrink-0 bg-red-500/20 text-red-600"><LockedIcon width={18} height={18} /></div>
            ...
            {clientLocked ? (
              <button className="flex items-center gap-3.5 px-4 py-3.5 rounded-md bg-bg-secondary border border-border-light text-sm font-medium text-text-secondary opacity-70 cursor-not-allowed text-left" disabled title={subscription.limitMessage('clients')}>
                <div className="w-9 h-9 rounded-sm flex items-center justify-center shrink-0 bg-emerald-500/20 text-emerald-600"><LockedIcon width={18} height={18} /></div>
              ...
              {recentInvoices.length === 0 ? (
                <tr>
                  <td colSpan={5} className="text-center text-text-secondary p-6">
                    Belum ada invoice. {invoiceLocked ? <span className="text-text-tertiary">Limit invoice sudah tercapai.</span> : <Link href="/invoices/create" className="text-primary">Buat invoice pertama →</Link>}
                  </td>
                </tr>
              ) : (
