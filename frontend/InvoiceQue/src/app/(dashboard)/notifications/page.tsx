...
import { Notification01Icon, MoneyBag01Icon, MoneySend01Icon, Alert01Icon, Link04Icon, User02Icon, NotificationBlock01Icon, ArrowLeft01Icon, ArrowRight01Icon, CheckmarkCircle01Icon, Delete01Icon, Cancel01Icon, Tick02Icon } from 'hugeicons-react'

export default function NotificationsPage() {
  const { notifications, unreadCount, loading, error, pagination, selectedIds, fetchNotifications, markAsRead, markAllAsRead, deleteNotification, deleteAllNotifications, deleteSelected, toggleSelect, selectAll, clearSelection, setPage } = useNotification();
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-text-primary">Notifikasi</h1>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {unreadCount > 0 && (
              ...
              >
                ...
                <span className="bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400 text-xs font-bold px-1.5 py-0.5 rounded-full">{unreadCount}</span>
              </button>
            )}
            <button
            ...
            {!confirmDeleteAll ? (
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => { deleteAllNotifications(); setConfirmDeleteAll(false); }}
      ...
      {selectionMode && (
        <div className="flex items-center gap-2 sm:gap-3 mb-4 py-2.5 px-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl animate-fade-in">
          ...
          <span className="text-sm font-medium text-text-primary whitespace-nowrap">
            {selectedIds.size > 0 ? `${selectedIds.size} dipilih` : 'Pilih notifikasi'}
          </span>
      ...
      {loading ? (
        ...
        <div className="card">
          <div className="flex flex-col items-center justify-center text-center py-16 px-5">
            ...
            <p className="text-text-secondary text-sm max-w-[360px] mx-auto mb-0 leading-[1.6]">Notifikasi akan muncul saat ada aktivitas seperti invoice terkirim, pembayaran diterima, dll.</p>
          </div>
        </div>
      ) : (
        <>
            ...
            {notifications.map((notif) => {
              ...
              return (
                ...
                >
                  ...
                  {selectionMode && (
                    <div className={`shrink-0 mt-1 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all duration-150 ${
                      isSelected 
                  ...
                  {/* Icon */}
                  <div className={`shrink-0 w-9 h-9 sm:w-11 sm:h-11 flex items-center justify-center rounded-xl text-lg sm:text-2xl ${
                    !notif.is_read 
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-0.5 gap-2">
                      <span className={`text-xs sm:text-sm text-text-primary truncate ${!notif.is_read ? 'font-bold' : 'font-semibold'}`}>
                        {getTypeLabel(notif.type)}
                      </span>
                      <span className="text-[11px] sm:text-xs text-text-tertiary whitespace-nowrap">{formatDate(notif.created_at)}</span>
                    </div>
                    <p className={`text-xs sm:text-[13px] leading-[1.5] m-0 line-clamp-2 ${!notif.is_read ? 'text-text-primary font-medium' : 'text-text-secondary'}`}>
                      {notif.message}
                    </p>
                    {notif.recipient && (
                      <span className="hidden sm:inline-block mt-1.5 text-xs text-text-tertiary bg-bg-secondary py-0.5 px-2 rounded-md">📧 {notif.recipient}</span>
                    )}
                  </div>
                  ...
                  {!selectionMode && (
                    <div className="shrink-0 flex items-center gap-1">
                      <span className={`text-sm ${notif.status === 'sent' ? 'opacity-50' : 'animate-pulse'}`}>
                        {notif.status === 'sent' ? '✅' : '⏳'}
                      </span>
                      <button
          ...
          {pagination.total_pages > 1 && (
            <div className="flex flex-col sm:flex-row items-center justify-between mt-5 gap-3">
              <p className="text-xs sm:text-sm text-text-tertiary order-2 sm:order-1">
                {((pagination.page - 1) * pagination.per_page) + 1}–{Math.min(pagination.page * pagination.per_page, pagination.total)} dari {pagination.total}
              </p>
              <div className="flex items-center gap-1 order-1 sm:order-2">
                ...
                {getPageNumbers().map((p, i) =>
                  p === '...' ? (
                    <span key={`e-${i}`} className="w-8 h-8 flex items-center justify-center text-text-tertiary text-xs select-none">…</span>
                  ) : (
                    <button
                      key={p}
