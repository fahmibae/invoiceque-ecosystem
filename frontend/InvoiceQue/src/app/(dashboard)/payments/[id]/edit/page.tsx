  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        setError(err instanceof Error ? err.message : t("paymentForm.loadError"));
      } finally {
        setLoading(false);
      }
    }
    ...
    <div className="animate-fade-in">
      <div className="page-header">
        <div className="page-header-left">
          <div className="flex items-center gap-2">
            ...
            <h1 className="page-title">{t("paymentForm.editTitle")}</h1>
          </div>
          <p className="page-subtitle">{t("paymentForm.editSubtitle")}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-6 items-start">
        <div className="flex flex-col gap-5">
          <div className="card">
            ...
            <div className="form-group">
              ...
              <input
                ...
                placeholder={t("paymentForm.previewTitleFallback")}
                value={title}
            ...
            <div className="form-row">
              ...
              <div className="form-group">
                <label className="form-label">
                  {t("paymentForm.amountLabel", { currency })}
                </label>
                <input
                  type="number"
                  className="form-input"
          ...
          <div className="card">
            <h3 className="flex items-center gap-2 text-base font-bold mb-4 pb-3 border-b border-border-light">
              <Settings01Icon /> {t("dashboard.status")}
            </h3>
            <div className="form-group">
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  className={`btn flex items-center gap-2 ${status === "active" ? "btn-primary" : "btn-secondary"}`}
                  onClick={() => setStatus("active")}
                >
                  <CheckmarkBadge01Icon /> {t("status.active")}
                </button>
                <button
                  type="button"
                  className={`btn flex items-center gap-2 ${status === "inactive" ? "btn-primary" : "btn-secondary"}`}
                  onClick={() => setStatus("inactive")}
                >
                  <PauseCircleIcon /> {t("status.inactive")}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col lg:flex-row justify-between items-center gap-2">
            <button
              className="btn btn-primary btn-lg w-auto"
            ...
            <Link
              ...
              className="btn btn-secondary lg:w-auto text-center"
            >
              {t("common.cancel")}
            </Link>
          </div>
          ...
          <div className="card p-0 overflow-hidden">
            ...
            <div className="px-6 py-8 flex flex-col items-center text-center">
              ...
              <p className="text-[13px] text-text-secondary mb-5 leading-[1.6]">
                {description || t("paymentForm.descriptionPlaceholder")}
              </p>
              ...
              <div
                className={`w-full p-3.5 font-bold text-[15px] rounded-md text-center mb-4 cursor-default ${status === "active" ? "bg-gradient-to-br from-red-600 to-red-500 text-white" : "bg-gray-300 text-gray-500"}`}
              >
                {status === "active"
