...
function formatPaymentCurrency(amount: number, currencyCode: string) {
  return new Intl.NumberFormat(currencyCode === "IDR" ? "id-ID" : "en-US", {
    ...
    minimumFractionDigits:
      currencyCode === "IDR" || currencyCode === "JPY" ? 0 : 2,
  }).format(amount);
}
...
function statusBadge(
  ...
  switch (status) {
    case "draft":
      return { label: "Draft", cls: "bg-gray-500/15 text-gray-500" };
    case "sent":
      return { label: "Terkirim", cls: "bg-blue-500/15 text-blue-600" };
    case "overdue":
      return { label: "Overdue", cls: "bg-red-500/15 text-red-500" };
    case "partial":
      return { label: "Bayar Sebagian", cls: "bg-amber-500/15 text-amber-600" };
    default:
      return { label: status, cls: "bg-gray-500/15 text-gray-500" };
  }
}
...
export default function CreatePaymentPage() {
  ...
  const handleInvoiceSelect = (invoiceId: string) => {
    ...
    if (inv) {
      ...
      setTitle(
        isRemainder
          ? `${t("invoices.outstanding.remainingPayment")} Invoice ${inv.number} - ${inv.client_name}`
          : isDpFirst
            ? `Down Payment Invoice ${inv.number} - ${inv.client_name}`
            : `${t("payments.payments")} Invoice ${inv.number} - ${inv.client_name}`,
      );
  ...
  const handleSubmit = async () => {
    ...
    if (!title.trim()) {
      setError(t("paymentForm.titleRequired"));
      return;
    }
    if (amount <= 0) {
      setError(t("paymentForm.amountRequired"));
      return;
    }
    const providerWarning = getCurrencyProviderWarning(
      currency,
      paymentProvider,
    );
    if (providerWarning) {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : t("paymentForm.createError"),
      );
    } finally {
      setSaving(false);
    }
  ...
  if (!subscription.loading && paymentLinkLocked) {
    return (
      <FeatureLimitLock
        ...
        backHref="/payments"
        backLabel={t("paymentForm.backToPayments")}
      />
    );
  }
      ...
      <div className="page-header">
        <div className="page-header-left">
          <div className="flex items-center gap-2">
            ...
            <h1 className="page-title">{t("paymentForm.createTitle")}</h1>
          </div>
          <p className="page-subtitle">{t("paymentForm.createSubtitle")}</p>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="card">
            <h3 className="flex items-center gap-2 text-base font-bold mb-4 pb-3 border-b border-border-light">
              <Invoice01Icon /> {t("paymentForm.linkInvoiceOptional")}
            </h3>

            {loadingInvoices ? (
              <div className="text-sm text-text-tertiary animate-pulse">
                {t("paymentForm.loadingInvoices")}
              </div>
              ...
              <div className="flex items-center gap-2 p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg text-blue-600 text-[13px]">
                <AlertCircleIcon /> {t("paymentForm.noPayableInvoices")}
              </div>
            ) : (
              <>
                <div className="form-group mb-0">
                  <label className="form-label">
                    {t("paymentForm.chooseInvoice")}
                  </label>
                  <select
                    ...
                    {linkableInvoices.map((inv) => {
                      ...
                      return (
                        <option key={inv.id} value={inv.id}>
                          ...
                          {getInvoiceAmountLabel(inv)} [{badge.label}]
                        </option>
                      );
                    })}
                  </select>
                ...
                {selectedInvoice &&
                  (() => {
                    ...
                    const hasPartialPaid =
                      isDP && selectedInvoice.amount_paid > 0;
                    return (
                      <div className="mt-3 p-3.5 rounded-lg bg-bg-secondary border border-border-light text-[13px]">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span
                            className={`inline-block py-0.5 px-2.5 rounded-full text-[11px] font-semibold ${badge.cls}`}
                          >
                            {badge.label}
                          </span>
                        </div>
                        ...
                        {isDP && (
                          <>
                            <div className="text-text-secondary mb-1">
                              DP ({selectedInvoice.dp_percentage}%):{" "}
                              <span className="text-text-primary font-medium">
                                {formatPaymentCurrency(
                                  selectedInvoice.dp_amount,
                                  "IDR",
                                )}
                              </span>
                            </div>
                            {hasPartialPaid && (
                              <>
                                ...
                                <div className="text-text-secondary">
                                  {t("invoices.outstanding.remainingBill")}:{" "}
          ...
          <div className="card">
            <h3 className="flex items-center gap-2 text-base font-bold mb-4 pb-3 border-b border-border-light">
              <GoogleDocIcon /> {t("paymentForm.detailTitle")}
            </h3>
            <div className="form-group">
              ...
              <input
                type="text"
                className="form-input"
                placeholder={t("paymentForm.titlePlaceholder")}
                value={title}
              ...
              <div className="form-group">
                ...
                {selectedInvoice && (
                  <p className="text-[11px] text-text-tertiary mt-1">
                    {t("paymentForm.invoiceAmountHint")}
                  </p>
                )}
              </div>
            </div>
            <div className="form-group">
              <label className="form-label">{t("paymentForm.expiryDate")}</label>
              <input
                type="date"
                className="form-input"
                value={expiryDate}
          ...
          <div className="card">
            <h3 className="flex items-center gap-2 text-base font-bold mb-4 pb-3 border-b border-border-light">
              <Settings01Icon /> {t("paymentForm.paymentMethod")}
            </h3>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
              {/* Xendit */}
              <label
                className={`flex flex-col items-center gap-2 py-4 px-3 border-2 rounded-lg cursor-pointer transition-all duration-150 hover:border-red-300 ${
                  paymentProvider === "xendit"
                    ? "!border-red-500 bg-gradient-to-br from-red-50 to-red-100/50 dark:from-red-900/20 dark:to-red-800/10"
                    : "border-border-color"
              ...
              {/* PayPal */}
              <label
                className={`flex flex-col items-center gap-2 py-4 px-3 border-2 rounded-lg cursor-pointer transition-all duration-150 hover:border-blue-300 ${
                  paymentProvider === "paypal"
                    ? "!border-blue-500 bg-gradient-to-br from-blue-50 to-blue-100/50 dark:from-blue-900/20 dark:to-blue-800/10"
                    : "border-border-color"
              ...
              {/* Manual */}
              <label
                className={`flex flex-col items-center gap-2 py-4 px-3 border-2 rounded-lg cursor-pointer transition-all duration-150 hover:border-gray-400 ${
            ...
            {currency !== "IDR" && !hasPaypal && (
              <div className="flex gap-2 items-center p-3 bg-amber-500/10 border border-amber-500/30 rounded-lg text-amber-700 dark:text-amber-400 text-[13px] mb-3">
                <Alert02Icon />{" "}
                {t("paymentForm.paypalRequired", { currency })}{" "}
            ...
            {getCurrencyProviderWarning(currency, paymentProvider) && (
              <div className="flex gap-2 items-start p-3 bg-red-500/10 border border-red-500/30 rounded-lg text-red-700 dark:text-red-400 text-[13px] mb-3">
                <Alert02Icon className="shrink-0 mt-0.5" />{" "}
                <span>
                  {getCurrencyProviderWarning(currency, paymentProvider)}
                </span>
              </div>
            ...
            <div className="flex justify-between items-center py-3.5 border-b border-border-light last:border-b-0">
              <div>
                <div className="text-sm font-semibold mb-0.5">
                  {t("paymentForm.multiplePayments")}
                </div>
                <div className="text-xs text-text-tertiary">
                  {t("paymentForm.multiplePaymentsHint")}
                </div>
              </div>
            ...
            <div className="flex justify-between items-center py-3.5 border-b border-border-light last:border-b-0">
              <div>
                <div className="text-sm font-semibold mb-0.5">
                  {t("paymentForm.emailNotifications")}
                </div>
                <div className="text-xs text-text-tertiary">
                  {t("paymentForm.emailNotificationsHint")}
                </div>
              </div>
              <label className="relative inline-block w-12 h-[26px] shrink-0">
                <input
            ...
            <div className="flex justify-between items-center py-3.5 border-b border-border-light last:border-b-0">
              <div>
                <div className="text-sm font-semibold mb-0.5">
                  {t("paymentForm.redirectAfterPayment")}
                </div>
                <div className="text-xs text-text-tertiary">
                  {t("paymentForm.redirectAfterPaymentHint")}
                </div>
              </div>
              <label className="relative inline-block w-12 h-[26px] shrink-0">
          ...
          <button
            ...
            disabled={saving}
          >
            {saving ? t("common.creating") : t("payments.create")}
          </button>
        </div>

        <div className="sticky top-[calc(var(--header-height)+24px)] max-lg:relative max-lg:top-0">
          <div className="card p-0 overflow-hidden">
            <div className="py-3 px-5 text-xs font-bold text-text-tertiary uppercase tracking-[0.5px] border-b border-border-light">
              Preview
            </div>
            <div className="px-6 py-8 flex flex-col items-center text-center">
              {logoCompany ? (
                <img
                  src={logoCompany}
                  alt="Logo"
                  className="w-auto h-20 object-cover mb-5"
                />
              ) : (
                <div className="w-[50px] h-[50px] bg-gradient-to-br from-red-600 to-red-500 rounded-md flex items-center justify-center font-extrabold text-lg text-white mb-5">
                  {initialCompany || "IQ"}
                </div>
              )}
              {selectedInvoice && (
              ...
              <h3 className="text-lg font-bold mb-2">
                {title || t("paymentForm.previewTitleFallback")}
              </h3>
              <p className="text-[13px] text-text-secondary mb-5 leading-[1.6]">
                {description || t("paymentForm.previewDescriptionFallback")}
              </p>
              <div
                className={`text-[28px] font-black bg-clip-text text-transparent mb-2 ${!accentColor ? "bg-gradient-to-br from-red-600 to-red-500" : ""}`}
              ...
              {selectedInvoice?.payment_type === "dp" &&
                ...
                selectedInvoice.amount_paid > 0 && (
                  <div className="text-[12px] text-amber-600 font-medium mb-2">
                    ...
                    {formatPaymentCurrency(selectedInvoice.total, "IDR")}
                  </div>
                )}
              {paymentProvider && (
                <div className="mb-4">
              ...
              <div
                className={`w-full p-3.5 text-white font-bold text-[15px] rounded-md text-center mb-4 cursor-default ${!accentColor ? "bg-gradient-to-br from-red-600 to-red-500" : ""}`}
                style={accentColor ? { backgroundColor: accentColor } : {}}
              >
