...
export default function TimeTrackingPage() {
  return (
    <PremiumGate feature="time_tracking">
      <TimeTrackingContent />
    </PremiumGate>
  );
}
...
function TimeTrackingContent() {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener(TIME_ENTRY_SAVED_EVENT, handleEntrySaved);
  }, [fetchData]);
  ...
  const formatDuration = (s: number) => {
    ...
    if (h > 0) return t("timeTracking.hoursMinutes", { hours: h, minutes: m });
    return t("timeTracking.minutes", { minutes: m });
  };
  ...
  const canToggleTimer =
    Boolean(activeSession) || Boolean(draftTaskTitle.trim());
    ...
    <div className="animate-fade-in">
      <div className="page-header">
        <div className="page-header-left">
          ...
          <p className="page-subtitle">{t("timeTracking.subtitle")}</p>
        </div>
      </div>
        ...
        <div className="p-6 flex items-center gap-6 flex-wrap">
          <div className="flex gap-5 min-w-[220px]">
            ...
            <input
              ...
              placeholder={t("timeTracking.customTaskPlaceholder")}
              value={taskValue}
              onChange={(e) => {
                setDraftTaskId("");
          ...
          >
            <option value="" style={{ color: "#000" }}>
              {t("timeTracking.projectPlaceholder")}
            </option>
            {projects.map((p) => (
          ...
          <div className="flex gap-3">
            <button
              ...
              style={{
                ...
                color: "#fff",
                cursor: canToggleTimer && !isSaving ? "pointer" : "not-allowed",
                opacity: canToggleTimer && !isSaving ? 1 : 0.5,
              }}
            >
      ...
      <div className="card">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border-light">
          <h3 className="text-base font-bold">{t("timeTracking.history")}</h3>
          <span className="text-xs text-text-tertiary">
            {t("timeTracking.entryCount", { count: entries.length })}
          </span>
        </div>
        {loading ? (
