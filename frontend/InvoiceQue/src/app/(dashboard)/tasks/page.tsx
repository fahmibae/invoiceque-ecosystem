...
const priorityConfig: Record<
  string,
  { labelKey: TranslationKey; bg: string; text: string }
> = {
  high: {
  ...
  const addTask = async () => {
    ...
    try {
      await taskApi.create({
        ...
        project_name: newTask.project_name || t("tasks.noProject"),
    ...
    <div className="animate-fade-in">
      ...
      <div className="page-header">
        <div className="page-header-left">
          <h1 className="page-title">{t("tasks.kanban.title")}</h1>
          <p className="page-subtitle">
            {t("tasks.kanban.subtitle", { total: totalTasks, done: doneTasks })}
          </p>
        </div>
        <div className="flex items-center gap-3">
          <Link href="/tasks/list" className="btn btn-secondary btn-sm">
            <FilterIcon width={16} height={16} /> {t("tasks.listView")}
          </Link>
          <button
            className="btn btn-primary"
            onClick={() => setShowModal(true)}
          >
            <Add01Icon width={16} height={16} /> {t("tasks.newTask")}
          </button>
        </div>
      </div>
            ...
            >
              <div className="flex items-center justify-between px-3 py-3 mb-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-xs font-semibold text-text-tertiary bg-bg-secondary px-2 py-0.5 rounded-full">
                    {tasks.length}
                  </span>
                </div>
                <button
                  className="w-7 h-7 flex items-center justify-center rounded-md hover:bg-bg-hover transition-colors text-text-tertiary hover:text-text-primary"
                  onClick={() => {
                    setNewTask((p) => ({ ...p, status: key }));
                    setShowModal(true);
                  }}
                >
              ...
              <div className="flex flex-col gap-3 flex-1">
                {tasks.map((task) => {
                  ...
                  const isDueSoon =
                    daysUntil !== null && daysUntil >= 0 && daysUntil <= 2;
                  const pri = priorityConfig[task.priority];

                  return (
                    <div
                      key={task.id}
                      className="bg-bg-card border border-border-color rounded-xl p-4 shadow-sm hover:shadow-md transition-all duration-200 hover:-translate-y-0.5 group relative"
                    >
                      <div className="flex items-start justify-between mb-2.5">
                        <span
                          className={`text-[11px] font-bold px-2.5 py-1 rounded-md ${pri.bg} ${pri.text}`}
                        >
                          {t(pri.labelKey)}
                        </span>
                            ...
                            <div className="absolute right-0 top-7 w-48 bg-bg-card border border-border-color rounded-lg shadow-lg z-50 py-1 animate-fade-in">
                              {columnOrder
                                ...
                                .map((k) => (
                                  <button
                                    key={k}
                                    className="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-text-secondary hover:bg-bg-hover hover:text-text-primary transition-colors"
                                    onClick={() => moveTask(task.id, k)}
                                  >
                                    <ArrowRight01Icon width={14} height={14} />{" "}
                                    {t("tasks.moveTo", {
                                      status: t(columnConfig[k].titleKey),
                                    })}
                                  </button>
                                ))}
                      ...
                      {task.tags && task.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mb-3">
                          {task.tags.map((tag) => (
                            <span
                              key={tag}
                              className="text-[10px] font-semibold px-2 py-0.5 rounded-md bg-red-50 text-red-600 dark:bg-red-900/20"
                            >
                              {tag}
                            </span>
                          ))}
                      ...
                      <div className="flex items-center justify-between pt-2.5 border-t border-border-light">
                        <div className="flex items-center gap-3 text-text-tertiary">
                          {task.due_date && (
                            <span
                              className={`flex items-center gap-1 text-[11px] font-medium ${isOverdue ? "text-red-500" : isDueSoon ? "text-amber-500" : ""}`}
                            >
                        ...
                        <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-bg-secondary text-text-tertiary truncate max-w-[100px]">
                          {task.project_name}
                        </span>
                      </div>
                      ...
                      {key === "done" &&
                        ...
                        !task.invoice_generated && (
                          <button
                            className="w-full mt-3 flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs font-bold text-red-600 bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors border border-red-200 dark:border-red-800/30"
                            onClick={() => openBillingModal(task)}
                          >
                            <GoogleDocIcon width={14} height={14} />
                            {t("tasks.generateDocument")} · {task.currency}{" "}
                            {(
                              (task.hourly_rate || 0) *
                              (task.estimated_hours || 0)
                      ...
                      {task.invoice_generated && (
                        <div className="w-full mt-3 text-center text-[11px] font-semibold text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg py-1.5">
                          {t("tasks.invoiceCreated")}
                        </div>
                      )}
                    </div>
                  );
            ...
            >
              <div className="flex items-center justify-between px-6 py-4 border-b border-border-light shrink-0">
                <h3 className="text-lg font-bold flex items-center gap-2">
                  <Task01Icon width={20} height={20} /> {t("tasks.newTask")}
                </h3>
                <button
              ...
              <div className="overflow-y-auto flex-1 px-6 py-5 flex flex-col gap-4">
                ...
                <div>
                  ...
                  <textarea
                    className="w-full py-2.5 px-3 border border-border-color rounded-lg bg-bg-secondary text-sm outline-none focus:border-red-400 transition-colors resize-none"
                    rows={3}
                ...
                <div>
                  ...
                  <select
                    className="w-full py-2.5 px-3 border border-border-color rounded-lg bg-bg-secondary text-sm outline-none focus:border-red-400"
                    value={newTask.currency}
                    onChange={(e) =>
                      setNewTask((p) => ({ ...p, currency: e.target.value }))
                    }
                  >
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-text-tertiary uppercase tracking-[0.5px] mb-2">
                      {t("tasks.hourlyRate", { currency: newTask.currency })}
                    </label>
                    <input
                      type="number"
                      className="w-full py-2.5 px-3 border border-border-color rounded-lg bg-bg-secondary text-sm outline-none focus:border-red-400 transition-colors"
                ...
                {newTask.hourly_rate && newTask.estimated_hours && (
                  <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/30 rounded-lg p-3 text-center">
                    <span className="text-xs text-text-tertiary">
                      {t("tasks.estimatedTotal")}{" "}
                    </span>
                    <span className="text-sm font-bold text-red-600">
                      {newTask.currency}{" "}
                      {(
                        Number(newTask.hourly_rate) *
                        Number(newTask.estimated_hours)
                      ).toLocaleString(intlLocale)}
                    </span>
                  </div>
