...
const columnConfig: Record<
  string,
  { titleKey: TranslationKey; color: string }
> = {
  backlog: { titleKey: "tasks.status.backlog", color: "#9CA3AF" },
  todo: { titleKey: "tasks.status.todo", color: "#3B82F6" },
  inprogress: { titleKey: "tasks.status.inprogress", color: "#F59E0B" },
  done: { titleKey: "tasks.status.done", color: "#10B981" },
};
...
export default function TaskDashboardPage() {
  ...
  const fetchTasks = useCallback(
    async ({ silent = false }: { silent?: boolean } = {}) => {
      ...
      try {
        ...
        setRefreshError(t("tasks.monitoringError"));
      } finally {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener(
        TIME_ENTRY_SAVED_EVENT,
        refreshAfterTimeEntrySaved,
      );
  }, [fetchTasks]);

  const allTasks = useMemo(
    () => columnOrder.flatMap((status) => tasksByStatus[status] || []),
    [tasksByStatus],
  );
  ...
  const getDaysUntilDue = useCallback(
    (due?: string) => {
      if (!due) return null;
      return Math.ceil(
        (new Date(due).getTime() - todayStart.getTime()) / 86400000,
      );
    },
    [todayStart],
  );
  ...
  const totalEstimatedValue = allTasks.reduce(
    (sum, task) =>
      sum +
      convertToIDR(
        task.hourly_rate || 0,
  ...
  const activeProjects = new Set(
    allTasks.map((task) => task.project_name).filter(Boolean),
  ).size;
  const highPriorityOpenTasks = allTasks.filter(
  ...
  const billingReadyTasks = allTasks
    .filter(
      (task) =>
        ...
        (task.hourly_rate || 0) > 0 &&
        (task.estimated_hours || 0) > 0,
    )
    .sort(
      (a, b) =>
        (b.hourly_rate || 0) * (b.estimated_hours || 0) -
        (a.hourly_rate || 0) * (a.estimated_hours || 0),
    )
    .slice(0, 6);
  const projectBreakdown = useMemo(() => {
    ...
    for (const task of allTasks) {
      const name = task.project_name || t("tasks.noProject");
  ...
  const lastUpdatedLabel = lastUpdatedAt
    ? lastUpdatedAt.toLocaleTimeString(intlLocale, {
        hour: "2-digit",
        minute: "2-digit",
        second: "2-digit",
      })
    ...
    <div className="animate-fade-in">
      <div className="page-header">
        <div className="page-header-left">
          <h1 className="page-title">{t("tasks.dashboard.title")}</h1>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            title={t("tasks.refreshMonitoring")}
          >
            <RefreshIcon
              width={16}
              height={16}
          ...
          <Link href="/tasks/list" className="btn btn-secondary btn-sm">
            {t("tasks.list.title")}
          </Link>
        </div>
      </div>
        ...
        <div className="bg-bg-card border border-border-color rounded-lg p-5 shadow-sm">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="relative flex h-2.5 w-2.5">
                  <span
                    className={`absolute inline-flex h-full w-full rounded-full ${autoRefreshEnabled ? "animate-ping bg-emerald-400 opacity-75" : "bg-text-tertiary opacity-30"}`}
                  />
                  <span
                    className={`relative inline-flex h-2.5 w-2.5 rounded-full ${autoRefreshEnabled ? "bg-emerald-500" : "bg-text-tertiary"}`}
                  />
                </span>
                <h3 className="text-base font-bold text-text-primary">
                  {t("tasks.liveMonitoring")}
                </h3>
              </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
            <div className="rounded-md border border-border-light bg-bg-secondary p-3">
              <div className="text-[11px] font-semibold uppercase tracking-[0.5px] text-text-tertiary">
                {t("tasks.active")}
              </div>
          ...
          <div className="rounded-md border border-border-light bg-bg-secondary p-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {inProgressTasks.slice(0, 4).map((task) => (
                <div
                  key={task.id}
                  className="rounded-md bg-bg-card px-3 py-2 border border-border-light"
                >
                  ...
                  <div className="mt-1 flex items-center justify-between gap-2 text-xs text-text-tertiary">
                    <span className="truncate">
                      {task.project_name || t("tasks.noProject")}
                    </span>
                    <span
                      className={
                        task.priority === "high"
        ...
        <div className="bg-bg-card border border-border-color rounded-lg p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-bold flex items-center gap-2">
              <Clock01Icon width={18} height={18} /> {t("tasks.activeTime")}
            </h3>
            <Link
              href="/time-tracking"
              className="text-xs font-semibold text-red-600 hover:underline"
          ...
          {activeSession ? (
            <div className="rounded-lg border border-red-200 bg-red-50 p-4 dark:border-red-800/30 dark:bg-red-900/20">
              <div className="text-sm font-bold text-text-primary line-clamp-2">
                {activeSession.taskTitle}
              </div>
              <div className="mt-1 text-xs text-text-tertiary line-clamp-1">
                {activeSession.projectName || t("tasks.noProject")}
              </div>
              <div className="mt-4 font-mono text-3xl font-extrabold text-red-600">
              ...
              <div className="mt-4 flex gap-2">
                ...
                >
                  ...
                  {isRunning ? t("tasks.pause") : t("tasks.resume")}
                </button>
                <button
                  type="button"
            ...
            <div className="flex min-h-[168px] flex-col items-center justify-center rounded-lg border border-dashed border-border-color bg-bg-secondary px-4 py-6 text-center">
              ...
              <div className="text-sm font-bold text-text-primary">
                {t("tasks.noActiveTime")}
              </div>
              <div className="mt-1 text-xs text-text-tertiary">
                {t("tasks.startTimeHint")}
              </div>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-bg-card border border-border-color rounded-lg p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs font-bold text-amber-600">
              {t("tasks.highPriorityCount", {
                count: highPriorityOpenTasks,
              })}
            </span>
          </div>
          <div className="text-2xl font-extrabold text-text-primary">
          ...
          <div className="text-xs text-text-secondary mt-4">
            {t("tasks.dueInSevenDays", { count: dueSoonTasks.length })}
          </div>
        </div>
        ...
        <div className="bg-bg-card border border-border-color rounded-lg p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs font-bold text-text-tertiary">
              {t("tasks.projectCount", { count: activeProjects })}
            </span>
          </div>
          <div className="text-2xl font-extrabold text-text-primary">
            {totalEstimatedHours.toLocaleString(intlLocale)}h
          </div>
          <div className="text-xs text-text-tertiary mt-1">
            {t("tasks.workEstimate")}
          </div>
          <div className="text-xs text-text-secondary mt-4">
            {t("tasks.inProgressCount", {
        ...
        <div className="bg-bg-card border border-border-color rounded-lg p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs font-bold text-emerald-600">
              {t("tasks.readyInvoiceCount", {
                count: billingReadyTasks.length,
              })}
            </span>
          </div>
          <div className="text-2xl font-extrabold text-text-primary">
            {formatCurrency(totalEstimatedValue, "IDR")}
          </div>
          <div className="text-xs text-text-tertiary mt-1">
            {t("tasks.valueEstimate")}
          </div>
          <div className="text-xs text-text-secondary mt-4">
            {t("tasks.valueEstimateHint")}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.2fr_1fr_1fr] gap-5 mb-6">
        <div className="bg-bg-card border border-border-color rounded-lg p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-bold flex items-center gap-2">
              <ChartIcon width={18} height={18} /> {t("tasks.statusProgress")}
            </h3>
            <span className="text-xs text-text-tertiary">
              {t("tasks.doneProgress", { done: doneTasks, total: totalTasks })}
            </span>
          </div>
                ...
                <div key={status}>
                  <div className="flex justify-between mb-1.5 text-xs">
                    <span className="font-semibold text-text-secondary">
                      {t(columnConfig[status].titleKey)}
                    </span>
                  ...
                  <div className="h-2.5 bg-bg-secondary rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${pct}%`,
                        background: columnConfig[status].color,
                      }}
                    />
                  </div>
                </div>
        ...
        <div className="bg-bg-card border border-border-color rounded-lg p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-bold flex items-center gap-2">
              <Calendar03Icon width={18} height={18} /> {t("tasks.upcomingDue")}
            </h3>
            <Link
              href="/calendar"
              className="text-xs font-semibold text-red-600 hover:underline"
        ...
        <div className="bg-bg-card border border-border-color rounded-lg p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-bold flex items-center gap-2">
              <GoogleDocIcon width={18} height={18} /> {t("tasks.readyInvoice")}
            </h3>
            <Link
          ...
          <div className="flex flex-col gap-2">
            {billingReadyTasks.length > 0 ? (
              billingReadyTasks.map((task) => {
                const amount =
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-1 text-xs font-semibold text-emerald-600">
                      {formatCurrency(amount, task.currency || "IDR")}
                    </div>
                  </button>
                );
              })
      ...
      <div className="bg-bg-card border border-border-color rounded-lg p-5 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-base font-bold flex items-center gap-2">
            <Folder01Icon width={18} height={18} /> {t("tasks.projectSummary")}
          </h3>
          <Link
            href="/projects"
            className="text-xs font-semibold text-red-600 hover:underline"
          >
            {t("tasks.manageProjects")}
          </Link>
        </div>
            ...
            {projectBreakdown.map((project) => {
              ...
              return (
                <div
                  ...
                  className="rounded-md border border-border-light bg-bg-secondary p-3"
                >
                  <div className="font-bold text-sm text-text-primary line-clamp-1">
                    {project.name}
                  </div>
                  <div className="flex justify-between mt-2 text-xs text-text-tertiary">
                    <span>
                      {t("tasks.projectProgress", {
                        done: project.done,
                        total: project.total,
                      })}
                    </span>
                  ...
                  <div className="mt-2 text-[11px] text-text-secondary">
                    {project.hours.toLocaleString(intlLocale)}h ·{" "}
                    {formatCurrency(project.value, "IDR")}
                  </div>
                </div>
              );
            })}
