  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        // Check if editable
        if (qt.status !== 'draft') {
          setError('Hanya quotation berstatus draft yang bisa diedit');
          setLoading(false);
          return;
        }
        ...
        const subtotal = qt.items.reduce((s: number, i: { quantity: number; price: number }) => s + i.quantity * i.price, 0);
        ...
        if (qt.items.length > 0) {
          setItems(qt.items.map((item: { description: string; quantity: number; price: number }, idx: number) => ({
            id: idx + 1,
        ...
        setError(err instanceof Error ? err.message : 'Gagal memuat quotation');
      } finally {
        setLoading(false);
      }
    }
  ...
  if (error && !selectedClient) {
    return (
      <div className="animate-fade-in p-10 text-center">
        ...
        <Link href="/quotations" className="btn btn-primary">Kembali ke Quotations</Link>
      </div>
    );
  }
      ...
      <div className="page-header">
        <div className="page-header-left flex items-center gap-2">
          <div className="flex items-center gap-2">
            <Link href={`/quotations/${quotationId}`} className="btn btn-icon btn-transparent border-none hover:bg-transparent hover:-translate-x-1 transition"><ArrowLeft02Icon /></Link>
            <h1 className="page-title">Edit Quotation</h1>
          </div>
          <p className="page-subtitle">Edit quotation {quotationNumber}</p>
        </div>
      </div>
        ...
        <div className="sticky top-[calc(var(--header-height)+24px)] max-lg:relative max-lg:top-0">
          <div className="card p-7">
            ...
            <div className="flex flex-col gap-2 mt-4">
              <button className="btn btn-primary btn-lg w-full" onClick={handleSubmit} disabled={saving}>{saving ? 'Menyimpan...' : 'Simpan Perubahan'}</button>
              <Link href={`/quotations/${quotationId}`} className="btn btn-secondary w-full text-center">Batal</Link>
            </div>
          </div>
        </div>
      </div>
