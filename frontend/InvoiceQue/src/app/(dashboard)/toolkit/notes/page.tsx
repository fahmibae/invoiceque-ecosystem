      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 min-w-0">
            <div className="p-2.5 rounded-xl bg-gradient-to-br from-amber-400 to-orange-500 shrink-0 shadow-lg shadow-amber-400/20">
              <StickyNote02Icon width={22} height={22} className="text-white" />
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-6">
            {pinnedNotes.map((note) => (
              <NoteCard
                ...
                emptyText={t("notes.emptyBody")}
                pinText={t("notes.pin")}
                unpinText={t("notes.unpin")}
            ...
            >
              <div className="flex items-center justify-between px-6 py-4 border-b border-border-light shrink-0">
                <h3 className="text-lg font-bold flex items-center gap-2">
                  ...
                  />
                  {editingId ? t("notes.editTitle") : t("notes.createTitle")}
                </h3>
                <button
                    ...
                    {NOTE_COLORS.map((c) => (
                      <button
                        ...
                        style={{
                          ...
                          borderColor:
                            color === c.value ? "#3B82F6" : "transparent",
                          transform:
    ...
    >
      <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <button
          ...
          title={note.is_favorited ? unpinText : pinText}
        >
          <PinIcon
            width={12}
            height={12}
            className={note.is_favorited ? "text-blue-600" : "text-gray-500"}
          />
        </button>
        <button
          className="p-1.5 rounded-lg bg-white/80 hover:bg-white shadow-sm transition-colors"
      ...
      <h3
        className={`font-bold text-sm mb-2 line-clamp-2 ${isDark ? "text-white" : "text-gray-800"}`}
      >
        {note.is_favorited && (
          <PinIcon
            width={12}
      ...
      <p
        className={`text-xs leading-relaxed line-clamp-5 whitespace-pre-wrap ${isDark ? "text-gray-300" : "text-gray-600"}`}
      >
        {body || emptyText}
      </p>
      <div
        className={`text-[10px] mt-3 ${isDark ? "text-gray-400" : "text-gray-400"}`}
      >
