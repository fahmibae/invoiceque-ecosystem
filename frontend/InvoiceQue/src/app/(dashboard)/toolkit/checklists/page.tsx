...
import {
  CheckListIcon, Add01Icon, Cancel01Icon, Search01Icon,
  Delete02Icon, MoreVerticalIcon, Loading03Icon,
  ArrowLeft01Icon, Tick01Icon, Copy01Icon,
...
export default function ChecklistsPage() {
  ...
  const [form, setForm] = useState({
    title: '', description: '', items: [{ text: '', checked: false }] as ChecklistItemData[], tags: '',
  });

  const fetchChecklists = useCallback(async () => {
    try {
      const res = await toolkitApi.list({
        type: 'checklist', search: search || undefined, per_page: 100,
      });
  ...
  const openCreate = () => {
    setEditingId(null);
    setForm({ title: '', description: '', items: [{ text: '', checked: false }], tags: '' });
    setShowModal(true);
  };
  ...
  const addItem = () => {
    setForm(p => ({ ...p, items: [...p.items, { text: '', checked: false }] }));
  };
    ...
    try {
      ...
      const data = {
        ...
        content: {
          ...
          items: validItems.length > 0 ? validItems : [{ text: 'Item baru', checked: false }],
        },
  ...
  const handleDelete = async () => {
    ...
    try { await toolkitApi.delete(deleteTarget); fetchChecklists(); } catch { /* error */ }
  ...
  return (
    <div className="animate-fade-in">
      <ConfirmModal isOpen={showDeleteModal} title="Hapus Checklist" message="Apakah Anda yakin ingin menghapus checklist ini?" confirmText="Hapus" onConfirm={handleDelete} onCancel={() => { setShowDeleteModal(false); setDeleteTarget(null); }} type="danger" />
      ...
      <div className="page-header">
        <div className="page-header-left">
          ...
          <p className="page-subtitle ml-[52px]">{checklists.length} checklist tersimpan</p>
        </div>
        <div className="flex items-center gap-3">
          <button className="btn btn-secondary" onClick={() => setShowTemplateModal(true)}>
            📋 Dari Template
          </button>
          <button className="btn btn-primary" onClick={openCreate} style={{ background: 'linear-gradient(135deg, #10B981, #0D9488)' }}>
            <Add01Icon width={16} height={16} /> Checklist Baru
          </button>
        </div>
      </div>
      ...
      {checklists.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-text-tertiary">
          ...
          <p className="font-bold text-lg mb-1">Belum ada checklist</p>
          <p className="text-sm mb-4">Buat checklist atau gunakan template yang sudah tersedia</p>
          <div className="flex gap-3">
            <button className="btn btn-secondary" onClick={() => setShowTemplateModal(true)}>📋 Dari Template</button>
            <button className="btn btn-primary" onClick={openCreate} style={{ background: 'linear-gradient(135deg, #10B981, #0D9488)' }}>
              <Add01Icon width={16} height={16} /> Buat Baru
            </button>
          </div>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {checklists.map(checklist => {
            ...
            return (
              <div key={checklist.id} className={`card group relative overflow-hidden transition-all duration-200 hover:shadow-lg ${isComplete ? 'ring-2 ring-emerald-400/30' : ''}`}>
                {/* Progress bar at top */}
                ...
                <div className="flex items-center justify-between mb-3 pt-2">
                  <div className="flex items-center gap-3">
                    <h4 className="text-sm font-bold text-text-primary">{checklist.title}</h4>
                    {isComplete && (
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-text-tertiary">{checkedItems}/{totalItems}</span>
                    <div className="relative">
                      <button
                        ...
                        onClick={() => setMenuOpen(menuOpen === checklist.id ? null : checklist.id)}
                      >
                      ...
                      {menuOpen === checklist.id && (
                        <div className="absolute right-0 top-8 w-44 bg-bg-card border border-border-color rounded-lg shadow-lg z-50 py-1 animate-fade-in">
                          <button className="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-text-secondary hover:bg-bg-hover transition-colors" onClick={() => openEdit(checklist)}>
                            ✏️ Edit
                          </button>
                          <button className="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-text-secondary hover:bg-bg-hover transition-colors" onClick={() => duplicateChecklist(checklist)}>
                            <Copy01Icon width={14} height={14} /> Duplikat (Reset)
                          </button>
                          <div className="border-t border-border-light my-1" />
                          <button className="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors" onClick={() => { setDeleteTarget(checklist.id); setShowDeleteModal(true); setMenuOpen(null); }}>
                ...
                {content?.description && (
                  <p className="text-xs text-text-tertiary mb-3">{content.description}</p>
                )}
                ...
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-border-light">
                  <div className="flex flex-wrap gap-1.5">
                    {(checklist.tags || []).map(tag => (
                      <span key={tag} className="text-[10px] font-semibold px-2 py-0.5 rounded-md bg-emerald-50 text-emerald-600 dark:bg-emerald-900/20">{tag}</span>
                    ))}
                  </div>
                  <span className="text-[11px] text-text-tertiary">
                    {new Date(checklist.updated_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}
                  </span>
                </div>
              </div>
            );
      ...
      {showTemplateModal && (
        <Portal>
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[1000] flex items-center justify-center p-5" onClick={() => setShowTemplateModal(false)}>
            <div className="bg-bg-card w-full max-w-lg rounded-2xl shadow-xl overflow-hidden border border-border-color animate-fade-in" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between px-6 py-4 border-b border-border-light">
                <h3 className="text-lg font-bold">📋 Pilih Template</h3>
                <button className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-bg-secondary transition-colors" onClick={() => setShowTemplateModal(false)}>
              ...
              <div className="p-4 flex flex-col gap-3 max-h-[60vh] overflow-y-auto">
                {TEMPLATES.map((tpl, idx) => (
                  ...
                  >
                    ...
                    <div>
                      <div className="font-bold text-sm text-text-primary">{tpl.title}</div>
                      <div className="text-xs text-text-tertiary mt-0.5">{tpl.description}</div>
                      <div className="text-[11px] text-text-tertiary mt-1">{tpl.items.length} item</div>
                    </div>
                  </button>
                ))}
              </div>
      ...
      {showModal && (
        <Portal>
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[1000] flex items-stretch sm:items-center justify-center p-0 sm:p-5" onClick={() => setShowModal(false)}>
            <div className="bg-bg-card w-full h-full sm:h-auto sm:max-w-[580px] sm:rounded-2xl sm:max-h-[90vh] shadow-xl overflow-hidden border border-border-color animate-fade-in flex flex-col" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between px-6 py-4 border-b border-border-light shrink-0">
                <h3 className="text-lg font-bold flex items-center gap-2">
                  <CheckListIcon width={20} height={20} className="text-emerald-600" />
                  {editingId ? 'Edit Checklist' : 'Checklist Baru'}
                </h3>
              ...
              <div className="overflow-y-auto flex-1 px-6 py-5 flex flex-col gap-4">
                <div>
                  ...
                  <input className="w-full py-2.5 px-3 border border-border-color rounded-lg bg-bg-secondary text-sm outline-none focus:border-emerald-400 transition-colors"
                    placeholder="Contoh: Pre-Deployment Checklist"
                ...
                <div>
                  <label className="block text-xs font-semibold text-text-tertiary uppercase tracking-[0.5px] mb-2">Checklist Items</label>
                  <div className="flex flex-col gap-2">
                    {form.items.map((item, idx) => (
                      <div key={idx} className="flex items-center gap-2">
                        <span className="text-xs font-bold text-text-tertiary w-5 text-center">{idx + 1}</span>
                        <input
                        ...
                        />
                        {form.items.length > 1 && (
                          <button className="w-7 h-7 flex items-center justify-center rounded-md hover:bg-red-50 dark:hover:bg-red-900/20 text-text-tertiary hover:text-red-500 transition-colors" onClick={() => removeItem(idx)}>
                  ...
                  <button className="mt-2 text-xs font-bold text-emerald-600 hover:text-emerald-700 flex items-center gap-1 transition-colors" onClick={addItem}>
                    <Add01Icon width={14} height={14} /> Tambah Item
                  </button>
                </div>
              </div>
              ...
              <div className="flex gap-3 px-6 py-4 border-t border-border-light shrink-0">
                ...
                <button className="btn btn-primary flex-1" onClick={saveChecklist} disabled={!form.title.trim() || saving} style={{ background: 'linear-gradient(135deg, #10B981, #0D9488)' }}>
                  {saving ? <Loading03Icon width={16} height={16} className="animate-spin" /> : <Add01Icon width={16} height={16} />}
                  {saving ? 'Menyimpan...' : editingId ? 'Simpan' : 'Buat Checklist'}
                </button>
              </div>
            </div>
          </div>
