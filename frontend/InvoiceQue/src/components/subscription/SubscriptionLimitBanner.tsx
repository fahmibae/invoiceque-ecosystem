...
export default function SubscriptionLimitBanner() {
  ...
  const modalKey =
    usage && lockedResources.length > 0
  ...
  const dismissModal = () => {
    ...
    setDismissedModalKeys((current) =>
      current.includes(modalKey) ? current : [...current, modalKey],
    );
  };
    ...
    <>
            ...
            <div className="min-w-0">
              <div className="mb-1 flex flex-wrap items-center gap-2">
                <h2 className="text-sm font-extrabold">
                  Limit plan {getPlanDisplayName(usage)} tercapai
                </h2>
              ...
              <p className="text-[13px] leading-relaxed text-red-800 dark:text-red-100/90">
                Fitur yang sudah mencapai batas akan terkunci. Kami juga
                mengirim email rekomendasi upgrade supaya Anda bisa memilih plan
                yang lebih pas.
              </p>
            </div>
          </div>
      ...
      {showModal && (
        <Portal>
          ...
          >
            <div
              className="w-full max-w-[520px] rounded-lg border border-border-color bg-bg-card shadow-2xl"
            ...
            >
              <div className="flex items-start justify-between gap-4 border-b border-border-light px-5 py-4">
                ...
                <button
                  ...
                  onClick={dismissModal}
                  aria-label="Tutup modal limit plan"
                >
              ...
              <div className="px-5 py-4">
                <div className="grid gap-2">
                  {lockedResources.map((resource) => {
                    ...
                    return (
                      <div
                        key={resource}
                        className="flex items-center justify-between rounded-md border border-border-light bg-bg-secondary px-3 py-2.5 text-sm"
                      >
                        <span className="font-semibold">
                          {resourceCopy[resource].label}
                        </span>
                ...
                <div className="mt-4 flex items-start gap-2 rounded-md bg-blue-500/10 px-3 py-2.5 text-[13px] text-blue-700 dark:text-blue-300">
                  ...
                  />
                  <span>
                    Email rekomendasi upgrade dikirim lewat Resend maksimal
                    sekali dalam 30 hari untuk limit yang sama.
                  </span>
                </div>
              </div>
