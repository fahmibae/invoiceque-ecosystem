...
export default function GsapDashboardMotion() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo(
        '[data-gsap-nav-item]',
        { autoAlpha: 0, x: -10 },
        { autoAlpha: 1, x: 0, duration: 0.38, stagger: 0.035, ease: 'power3.out', delay: 0.12 }
      );
    });
  ...
  useEffect(() => {
    ...
    const pageChildren = gsap.utils.toArray<HTMLElement>('[data-gsap-page] > *');
    ...
    hoverItems.forEach((item) => {
      const onEnter = () => {
        gsap.to(item, { y: -2, scale: 1.015, duration: 0.22, ease: 'power2.out' });
      };
      const onLeave = () => {
