...
import { subscriptionApi, type SubscriptionResource, type UsageData } from '@/lib/api';
import { getLimitReachedMessage, getLockedResources, isResourceLocked as readResourceLocked } from '@/lib/subscription-limits';
...
export function useSubscriptionUsage() {
  ...
  const loadUsage = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Gagal memuat penggunaan plan');
      setUsage(null);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (!usage || lockedResources.length === 0 || typeof window === 'undefined') return;
