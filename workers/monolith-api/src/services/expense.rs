...
pub async fn create_expense(req: Request, env: &Env, claims: &JwtClaims) -> Result<Response> {
    ...
    let item: Option<Expense> = db
        .query_one(
            &format!(
                "INSERT INTO expenses (user_id, project_id, client_id, category, title, description, amount, currency, expense_date, receipt_url, is_tax_deductible, is_recurring, recurring_interval, tags) VALUES ($1::uuid, NULLIF($2, '')::uuid, NULLIF($3, '')::uuid, $4, $5, $6, $7, $8, COALESCE($9::date, CURRENT_DATE), $10, $11, $12, $13, $14::jsonb) RETURNING {}",
                EXPENSE_COLS
            ),
            &[
