...
fn db(http: &reqwest::Client) -> Result<crate::db::NeonClient, AppError> {
    utils::get_db("SUBSCRIPTION_DB_URL", http)
}
...
pub async fn get_usage(
    ...
    match sub {
        Some(s) => utils::json_response(
            &serde_json::json!({"invoices_used": s.invoices_used, "clients_used": s.clients_used, "payment_links_used": s.payment_links_used}),
            200,
        ),
        None => utils::json_response(
            &serde_json::json!({"invoices_used": 0, "clients_used": 0, "payment_links_used": 0}),
            200,
        ),
    }
}
...
pub async fn create_checkout(
    ...
    let base_url = utils::get_env("XENDIT_BASE_URL");
    ...
    let payload = serde_json::json!({
        ...
        "amount": plan.price,
        "description": format!("Langganan InvoiceQu - Paket {}", plan.display_name),
        "currency": plan.currency,
        "success_redirect_url": "https://app.invoicequ.my.id/subscription",
        "failure_redirect_url": "https://app.invoicequ.my.id/subscription",
        "invoice_duration": 86400,
    });

    let resp = http
        .post(&format!("{}/v2/invoices", base_url))
    ...
    if !status.is_success() {
        return utils::json_response(
            &serde_json::json!({"error": "Gagal membuat invoice langganan di Xendit", "details": result}),
            502,
        );
    }
    ...
    let checkout_url = match result.get("invoice_url").and_then(|v| v.as_str()) {
        ...
        _ => {
            return utils::json_response(
                &serde_json::json!({"error": "Gagal mendapatkan URL checkout Xendit", "details": result}),
                502,
            )
        }
    };
    ...
    utils::json_response(
        &serde_json::json!({"external_id": external_id, "plan": plan, "checkout_url": checkout_url}),
        200,
    )
}
...
pub async fn update_plan(
    ...
    if let Some(v) = body.get("max_invoices").and_then(|v| v.as_i64()) {
        db.execute(
    ...
    if let Some(v) = body.get("max_clients").and_then(|v| v.as_i64()) {
        db.execute(
    ...
    if let Some(v) = body.get("max_payment_links").and_then(|v| v.as_i64()) {
        db.execute(
