...
pub async fn list(
    ...
    let page = utils::query_param(qs, "page")
        .and_then(|v| v.parse::<i32>().ok())
        .unwrap_or(0);
    let size = utils::query_param(qs, "size")
        .and_then(|v| v.parse::<i32>().ok())
        .unwrap_or(10);
    ...
    utils::json_response(
        &serde_json::json!({"data": invoices, "total": total, "page": page, "per_page": size, "total_pages": total_pages}),
        200,
    )
}
...
pub async fn create(
    ...
    let mut inv: Option<Invoice> = db
        ...
        .await?;
    if let Some(ref mut invoice) = inv {
        invoice.items = db.query_typed("SELECT id, invoice_id, description, quantity, price, total FROM invoice_items WHERE invoice_id = $1", &[serde_json::json!(id)]).await.unwrap_or_default();
    }
    match inv {
...
pub async fn get_dashboard_stats(
    ...
    let paid_invoices: i64 = db
        ...
        .await
        .unwrap_or(0);
    let pending_amount: f64 = db.query_scalar("SELECT COALESCE(SUM(amount_remaining),0) FROM invoices WHERE user_id=$1 AND status IN ('sent','outstanding','partially_paid')", &[uid.clone()]).await.unwrap_or(0.0);
    let overdue_invoices: i64 = db
        ...
        .unwrap_or(0);
    utils::json_response(
        &serde_json::json!({"totalRevenue": total_revenue, "totalInvoices": total_invoices, "paidInvoices": paid_invoices, "pendingAmount": pending_amount, "overdueInvoices": overdue_invoices, "activePaymentLinks": 0}),
        200,
    )
}
